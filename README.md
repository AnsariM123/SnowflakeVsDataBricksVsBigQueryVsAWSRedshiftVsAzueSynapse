...
Storage layer — cheap, elastic cloud object storage (S3/Azure Blob/GCS under the hood).
Compute layer — independent "virtual warehouses" that can scale up/down or spin up/down instantly.
Cloud services layer — handles metadata, security, and query optimization.
...
Delta Lake — added reliability (ACID transactions) to raw data lakes, solving the "data swamp" problem where lakes became messy and untrustworthy.
The Lakehouse architecture — merged the flexibility of a data lake with the structure/performance of a warehouse, letting one platform serve both data engineers and BI analysts.
MLflow — became the industry standard for tracking ML experiments and managing model lifecycles.
Unity Catalog — added enterprise-grade governance across data and AI assets.
Most recently: deep generative AI/LLM tooling (Databricks Mosaic AI) for building and fine-tuning custom models.
Why This History Matters
...
Redshift Spectrum — allowed querying data directly in S3 without loading it in, bridging the warehouse/data-lake gap.
RA3 nodes — introduced separated storage/compute (playing catch-up with Snowflake's core innovation).
Redshift Serverless — added a pay-per-use option for variable workloads.
Zero-ETL integrations — direct pipelines from Aurora, RDS, and other AWS services straight into Redshift.
Why This History Matters
...
Need serious ML, data science, or unstructured data processing? → Databricks

Need flexible, multi-cloud SQL analytics with easy data sharing across teams or external partners? → Snowflake

Want zero infrastructure management and pay only for what you query? → BigQuery

Need one workspace combining warehousing, Spark, and BI without picking multiple vendors? → Azure Synapse
